...
var OfekQuery = function (query) {
	var bodyElements = document.documentElement.childNodes;
	this.elements = [];
...
OfekQuery.prototype = {
	...
	filterElemnts: function (elements, elementsToCheck, queryArray)
	{
		...
		{
			filterdElements.forEach(x=> elements.push(x));
			filterdElements.forEach(element => OfekQuery.prototype.filterElemnts(elements, element.childNodes, queryArray));
		}

		filterdElements.forEach(element => OfekQuery.prototype.filterElemnts(elements, element.childNodes,queryArray.slice(1)));
		unfilteredElements.forEach(element => OfekQuery.prototype.filterElemnts(elements, element.childNodes, queryArray));
	},
